...
grep -E "(TO-DO|FIXME|XXX|HACK|PLACEHOLDER)" "$file"
...
grep -E "^$VAR_NAME=.+" .env .env.local 2>/dev/null | grep -v "your-.*-here|xxx|placeholder|TO-DO" -i
...
check_stubs() {
  local file="$1"
  local stubs=$(grep -c -E "TO-DO|FIXME|placeholder|not implemented" "$file" 2>/dev/null || echo 0)
